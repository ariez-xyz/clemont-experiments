...
for epsilon in $(seq -f "%.3f" 0.005 0.005 0.200); do
  ...
  printf 'Base model epsilon %s (max-k none)|%s|base_eps_%s_maxk_none|--epsilon %s\n' \
    "${epsilon}" "${BASE_MODEL}" "${epsilon_token}" "${epsilon}" >> "${CONFIG_FILE}"
done
