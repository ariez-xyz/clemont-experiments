...
DATA_DIR="$PROJECT_ROOT/data/text/toxic-chat"
RESULTS_DIR="$PROJECT_ROOT/results/quantitative/text_toxic_chat/5class_4b_probs_revised"
...
paths = list(root.glob("toxic-chat-judge-*_5class_*_witness_revised_*.csv"))
if not paths:
    raise SystemExit(1)
print(max(paths, key=lambda path: path.stat().st_mtime))
PY
)" || {
  echo "No revised ToxicChat CSV found under $DATA_DIR" >&2
  exit 1
}
