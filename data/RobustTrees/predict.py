# This file was adapted from RobustTrees/xgbKantchelinaAttack.py. Most of the code is unused as we aren't running attacks, only predictions.
...
def main(args):
    ...
    header = ['pred'] + [str(i) for i in range(test_data.shape[1])]
    
    with open(outfile, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        for idx in range(len(test_data)):
            predict = model.predict(test_data[idx])[0]
            row = [predict] + test_data[idx].tolist()
            writer.writerow(row)
...
if __name__ == '__main__':
	...
	parser.add_argument('-f', '--file_name', type=str, required=True, help='output filename')
